  ...
  return (
    <div className="flex min-h-screen">
      ...
      <aside className="w-[240px] shrink-0 flex flex-col fixed top-0 left-0 h-screen z-40"
        ...
        {/* ── User + Logout (visible to ALL roles) ── */}
        <div className="px-3 pb-4 pt-2 shrink-0" style={{ borderTop: '1px solid #253047' }}>

          {/* Profile row */}
          <div
            onClick={() => navigate('/profile')}
            className="flex items-center gap-3 px-3 py-2.5 rounded-xl cursor-pointer transition hover:bg-white/5 mb-1"
          >
          ...
          {/* Logout button — always visible for ALL roles */}
          <button
      ...
      <div className="flex-1 flex flex-col min-w-0 ml-[240px]" style={{ backgroundColor: '#f3f4f8' }}>
        ...
        <header className="h-14 bg-white flex items-center justify-between px-7 sticky top-0 z-30"
          ...
          <div className="flex items-center gap-2">
            ...
            {/* Bell — explicit dark color so always visible on white */}
            <div className="relative">
              <button
                onClick={() => setShowNotif(!showNotif)}
                className="w-9 h-9 rounded-xl flex items-center justify-center transition hover:bg-gray-50"
                style={{ border: '1px solid #e8eaf0', backgroundColor: '#fff', color: '#374151' }}
                aria-label="Notifications"
              >
                <i className="ti ti-bell text-[18px]" style={{ color: '#374151' }} />
                {unread > 0 && (
                  <span className="absolute -top-1 -right-1 w-[17px] h-[17px] bg-red-500 text-white text-[10px] rounded-full flex items-center justify-center font-bold"
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
              {filteredProjects.map(project => {
                ...
                const progress = project.status === 'completed'
                  ? 100
                  : Math.min(Math.round((taskCount / 5) * 100), 95)
